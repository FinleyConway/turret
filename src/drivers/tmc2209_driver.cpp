...
namespace turret {
    ...
    bool tmc2209_driver::rotate(uint16_t angle) {
    ...
    void tmc2209_driver::set_direction(direction direction) {
        m_direction = direction;
    }

    bool tmc2209_driver::set_speed(uint64_t rpm) {
    ...
    void tmc2209_driver::enable(bool enable) {
        m_enabled = enable;

        gpio::write(c_spec.enable_pin, !enable);
    }

    constexpr uint64_t tmc2209_driver::angle_to_step(uint16_t angle) const {
        return (static_cast<uint64_t>(angle) * c_spec.steps_per_rev) / 360u;
    }

    constexpr std::chrono::microseconds tmc2209_driver::rpm_to_step_period(uint64_t rpm) const {
